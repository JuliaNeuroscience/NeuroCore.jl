...
and so on. Combined with `color_view`, this allows one to combine two
...
# For getting all of the channels (e.g., for color_view)
...
# For setting all of the channels (e.g., for color_view), one `val` per parent
